#Experimental Hub Script for Downloading and Filtering RNA Seq Gene Expression data from Bgee

#install and load BgeeDB as well as other package that are needed to build this data frame (make sure that all packages are up to data)
...
#This function downloads species specific data from Bgee, filters for necessary necessary columns, and creates a column of the Variance Stabilized Transformations of Read Counts for this data. 
Bgee_Download <- function(species,dataType){
  ...
  species_db$Anatomical.entity.name<-gsub('"',"",species_db$Anatomical.entity.name) # remove the quotations from the anatomical entities
  species_specific<- data.frame(dplyr::select(species_db, Library.ID, Experiment.ID,Gene.ID, Anatomical.entity.ID, Anatomical.entity.name, TPM, Read.count)) # filter the bgee data by these columns
  VST_calc <- species_db %>% select(Gene.ID, Library.ID, Anatomical.entity.name, Read.count)
  VST_calc$Library.ID_Tissue <- paste(VST_calc$Library.ID, VST_calc$Anatomical.entity.name, sep = "_")
  VST_calc <- VST_calc %>% select(Gene.ID, Library.ID_Tissue, Read.count)
  VST_calc <-VST_calc %>% tidyr::pivot_wider(names_from = Library.ID_Tissue, values_from = Read.count) %>% column_to_rownames(var = "Gene.ID") %>% as.matrix() %>% round(.,0)
  VST_matrix <- as.data.frame(DESeq2::vst(VST_calc))
  VST_matrix <- tibble::rownames_to_column(VST_matrix, "Gene.ID")
  VST_df <- pivot_longer(VST_matrix, cols=-1, names_to = "Library.ID_Tissue", values_to = "VST")
  ...
  species_specific<-merge(VST_df,species_specific,by = c("Gene.ID","Library.ID","Anatomical.entity.name"))
  return(species_specific)
}

#This function organizes the species specific data from above into the correct format for Experimental Hub.
Species_Dataframe <- function(dfBgee, species_name){
  Species<-dfBgee %>% group_by(Gene.ID,Anatomical.entity.name) %>% summarise(Sample_size = n(), Median_VST = median(VST), Scaled_Median_VST = median(scale(VST, center = FALSE)),VST = paste(unique(VST), collapse = ', '), Experiment_ID = paste(unique(Experiment.ID), collapse = ', '))
  Species_tissue<-dfBgee %>% select(Anatomical.entity.ID, Anatomical.entity.name)
...
GEx_Bulk_Bgee_Nematode<-Species_Dataframe(nematode_specific, "Caenorhabditis_elegans")
